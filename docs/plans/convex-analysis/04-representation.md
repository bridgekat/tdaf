...
| `helly_of_polyhedral_tail` | Thm 21.4, 21.5 | still blocked, but on **different** pieces — see below |
...
**What is left of Theorems 21.4 and 21.5.** Two pieces, both in §19:

* **Corollary 19.1.2 for *functions***: `k₀`, the positively homogeneous convex function generated
  by `conv {fᵢ* ∣ i ∈ I₀}` for affine `fᵢ`, is polyhedral. `epi k₀` is the convex cone generated by
  the finitely many points `(aᵢ, αᵢ)` together with `(0, 1)`; Minkowski–Weyl is in
  `Polyhedral/Cone.lean`, but nothing identifies `posHomGenCone (convFn …)` with that cone.
* The identification `kⱼ* = δ(· ∣ Cⱼ)` and the separation step `(-dom k₀) ∩ ri (dom k₁) ≠ ∅`
