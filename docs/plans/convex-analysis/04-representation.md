...
| `convexHull_iUnion_eq_affineIndependent_combination` | Cor 17.1.1–17.1.2 | not done — see below |
| `convFn_eq_finite_combination` | Cor 17.1.3–17.1.6 | not done — see below |
| `IsCompact.isCompact_convexHull`, `Bornology.IsBounded.closure_convexHull` | **Thm 17.2**, Cor 17.2.1 | done |
| `convFn_of_compact_epi` | Thm 17.3 | not started |
...
(**Thm 17.2**). Corollaries 17.1.1–17.1.6 and Theorem 17.3 are not done.
...
| `closure_convexHull_exposed` | Thm 18.7, Cor 18.7.1 | **not done** |
| `eq_iInter_tangent_halfspaces` | Thm 18.8 | **not done** |
...
| `recessionCone_polyhedral_system`, `Polyhedral.polyhedralCone_recessionCone` | **Thm 19.5** | done |
...
| `alternative_infinite_system` | **Thm 21.3**, Cor 21.3.1 | blocked — see below |
| `helly_of_no_common_recession` | **Cor 21.3.2** | blocked — see below |
| `helly_of_polyhedral_tail` | Thm 21.4, 21.5 | blocked — see below |
...
**Theorems 21.3–21.5 and Corollaries 21.3.1–21.3.2 are blocked upstream, not by §21.** Theorem 21.3
is the alternative for *weak* inequalities and arbitrary index sets; its proof runs through the
positively homogeneous convex function generated by `conv {fᵢ* | i ∈ I}`, whose conjugate is
identified by **Theorem 13.5** (deferred in this development), and it extracts the multipliers by
**Corollary 17.1.3**, the *directions* half of Carathéodory's theorem (§17 — only the point case is
done). Corollaries 21.3.1 and 21.3.2 are corollaries of it; Theorems 21.4 and 21.5 weaken its
recession hypothesis using §20. Doing Cor 17.1.3 (sub-plan §4.1) and Thm 13.5 unblocks the whole
group at once.
