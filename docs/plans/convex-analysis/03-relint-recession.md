...
9.2.2 is here too; Theorems 9.6–9.8 with Corollaries 9.6.1 and 9.8.1 are in
`Recession/ConeHull.lean`, which also supplies the *convex cone generated by a set* (it is
`PointedCone.hull ℝ`, so nothing new had to be defined) and the `cl K` formula of Theorem 8.2 that
`Recession/Cone.lean` deliberately skipped. Corollary 9.2.1 and the `m`-fold forms of 9.8/9.8.1 are
deferred as contentless inductions; Corollary 9.7.1 is left to §15 because it is a statement about
gauges; Corollary 9.8.2 is superseded by `Caratheodory.lean`; Corollary 9.8.3 needs a `convHullFn`
for a finite *family*, which is §5 work.
...
| `closedProperConvexFn_infConv`, `exists_add_eq_of_infConv_le` — **done** | **Cor 9.2.2**, formalized as `epi (f □ g) = epi f + epi g`, from which "the infimum is attained" falls out. Cor 9.2.1 deferred |
