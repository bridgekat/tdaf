...
`epi_convFn₂` is `epi_convFn` for two functions, and `IsEpiLike.mem_convexHull_of_le` /
`isEpiLike_convexHull_epi_union` are what discharge its `IsEpiLike` hypothesis: the convex hull of
an epi-like set is *always* upward closed in the vertical coordinate, so a convex hull of epigraphs
fails to be an epigraph only by failing to be closed. That is the algebraic half of Corollary
9.8.3; the closedness half is `Recession/ConeHull.lean`.
...
`forall_mem_linealitySpace_prod`. **Cor 9.2.1** for two functions as
`closedProperConvexFn_infConv_of_recessionFn_symm` and
...
theorem closedProperConvexFn_convFn₂ …, exists_combo_of_convFn₂_le …  -- Cor 9.8.3
```

**Cor 9.8.3** is Cor 9.8.1 on the two epigraphs plus `isEpiLike_convexHull_epi_union`
(`Operations/Hull.lean`), which is why this file imports `Operations/Hull.lean`. The epigraph
identity `epi (conv {f, g}) = conv (epi f ∪ epi g)` *is* the attainment of the infimum in Theorem
5.6, read off through `Convex.convexHull_union` exactly as `convFn₂_apply` does; and properness is
read off the recession cone — a vertical line would put `(0, -1)` in `epi k`, where `k 0 = 0`.
