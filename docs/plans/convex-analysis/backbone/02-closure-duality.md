...
| `partialAffineFn`, `conj_partialAffineFn` — **done**, in `Duality/Polar.lean` | the partial affine functions: `f = δ(· \| L + a) + ⟨·, a*⟩ + α` has `f* = δ(· \| L^⊥ + a*) + ⟨·, a⟩ + α*`, `α* = -α - ⟨a, a*⟩` | **§12**, the display preceding Thm 12.3. It is `conj_comp_affine` at `h = δ(· \| L)`, `A = I`, fed by `conj_indicatorFn_eq_indicatorFn_polarCone`; the only new ingredient is `indicatorFn_vadd`, `δ(x \| a + s) = δ(x - a \| s)`, now in `Indicator.lean`. It lives in `Duality/Polar.lean` and not in `Duality/Conjugate.lean` because the dual datum is a *polar*: the statement cannot be written before `polarCone` exists — which is Rockafellar’s own remark that the subspace example "will be broadened at the beginning of §14". Stated so that the two sides are the **same** construction read through the polar pairing, `conj B (partialAffineFn B L a b α) = partialAffineFn B.flip (polarSubmodule B L) b a (-α - ⟨a, b⟩)`, which is the sense in which partial affine functions come in dual pairs. **The Tucker-representation form is still not done**: it is a statement about coordinates on `Rᴺ` and belongs to the surface layer |
...
**Status: §14 and §15 are done, Theorem 15.3 included.** Theorems 14.6 and 14.7 and all of
...
| `monotoneComp`, `conj_monotoneComp`, `closedProperConvexFn_monotoneComp`, `PosHomogeneousDeg`, `degGauge`, `posHomogeneousDeg_iff_exists_isGauge`, `conj_monotoneComp_powHalfLine`, `polarGauge_degGauge`, `pairing_le_rpow_mul_rpow`, `polarSet_setOf_le_inv` — **done**, in `Duality/GaugeLike.lean` | **Thm 15.3**'s first assertion and conjugacy formula `(g ∘ k)* = g⁺ ∘ k°`, and **Cors 15.3.1–15.3.2** in full, Hölder included. Beyond Thm 12.4 it needed the Thm 8.6 growth estimate for a non-constant convex function of the half-line and right-continuity at the origin — and, for the first assertion only, Fenchel–Moreau, hence a compatible pairing, a continuous flip and local convexity of `E`; the conjugacy formula needs none of those. **The converse half is now done too** — see the next row |
| `IsGaugeLike`, `IsGaugeLike.exists_isGauge_setOf_le_eq`, `IsGaugeLike.exists_eq_monotoneComp`, `isGaugeLike_monotoneComp`, `closedProperConvexFn_and_isGaugeLike_iff`, `isGaugeLike_conj_monotoneComp` — **done**, in `Duality/GaugeLike.lean` | **Thm 15.3** in full: gauge-like closed proper convex ⇔ `g ∘ k`. The reconstruction needs **no** pairing and no local convexity — only a topological vector space — because it never conjugates; the pairing enters the final `iff` solely through the forward implication’s closedness. The recorded blocker, convexity of the reconstructed `g`, was not the obstacle and neither was the level-set bookkeeping: **one** lemma does the work, "every sublevel set of `f` above `inf f` is a sublevel set of `k`" (`IsGaugeLike.exists_isGauge_setOf_le_eq`, with `k` the gauge of `{f ≤ f 0 + 1}`), from which `k u ≤ k v ⇒ f u ≤ f v` and `k u = ⊤ ⇒ f u = ⊤` follow, so that `f x` depends on `x` only through `k x`. Rockafellar’s case split survives, but only to choose `g`: on the ray, `g ζ = f (ζ • x₁)` with `k x₁ = 1` — whose convexity and closedness are `f`’s, transported by `compLin` and `restrict` — and in the degenerate case where `k` takes only the values `0` and `⊤`, the step function `f 0 + δ(· \| [0, 1])`. Non-constancy and finiteness at a positive level, which Rockafellar imposes on `g`, are *produced*, not assumed. The theorem's **second assertion**, "`f*` is gauge-like too", is `isGaugeLike_conj_monotoneComp`: it needed the missing half of "`g⁺` satisfies the same conditions as `g`", namely `exists_lt_monotoneConj`, `g` finite at a positive level ⇒ `g⁺` non-constant — conjugacy *exchanges* the two side conditions |
