...
| 12.22 | **An `EReal` infimum over a product does not split anywhere public.** Wanted in `Order/EReal.lean`: `iInf_prod_add`, with `add_iInf` / `iInf_add` / `coe_add_iInf` beside it. `Order/EReal.lean` has only the *supremum* form `biSup_add_biSup`, whose hypothesis is on the values rather than on the extrema; `Bifunction/Algebra.lean` has the `≠ ⊥` variants and all three are `private`. Both finiteness hypotheses are necessary — `ψ i = -i` on `ℕ` and `φ ≡ ⊤` break it | §31 | open — one of the two things that made Theorem 31.2 expensive |
