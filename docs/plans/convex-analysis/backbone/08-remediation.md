...
| 11.19 | **`posHomGen_mono` is declared twice, publicly, in the same namespace** — `HellyRefined.lean:196` and `Subgradient/Approx.lean:284`, the same statement proved twice. Neither module imports the other, so nothing is ambiguous *yet*; any module importing both would find the bare name unusable. It belongs once, in `Duality/Level.lean` where `posHomGen` is defined | sweep | open — pre-existing, and the only public collision in 4 751 declarations. **The stated home is wrong**: `posHomGen` is defined in `Recession/ConeHull.lean`, and `Duality/Level.lean`’s own docstring says so. The lemma is a three-line consequence of `le_posHomGen`, `posHomGen_le`, `convexFn_posHomGen`, `posHomogeneous_posHomGen` and `posHomGen_apply_zero_le` — all five in `Recession/ConeHull.lean`, whose section variables are exactly the ones both copies use — and both consumers import it transitively. It goes immediately after `le_posHomGen`; the two copies are `HellyRefined.lean:194–198` and `Subgradient/Approx.lean:283–286` |
