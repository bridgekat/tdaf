...
| 9.18 | **`m`-ary sums are binary throughout `Recession/`.** `recessionCone_prod` / `linealitySpace_prod` exist for binary products but not for `Set.pi`, and the book’s own proof of Corollary 9.1.1 runs Theorem 9.1 on `C₁ ⊕ ⋯ ⊕ Cₘ ⊆ ℝᵐⁿ`. §9 states Corollaries 9.1.1, 9.1.3, Theorem 9.3 and Theorem 9.8 for two sets | §9 | **half done** — `recessionCone_pi`, `linealitySpace_pi`, `pi_recessionCone_subset` are in `Recession/Cone.lean`. What is still missing is the *sum map* `(xᵢ) ↦ ∑ xᵢ` against a `Set.pi`, an `m`-ary `infConv`, and `recessionFn` of a separable sum — the book’s own route. The §9 statements stay binary |
...
| 11.14 | **Corollary 16.2.2 is a product-of-`ι` problem, not a §16 one.** It is Lemma 16.2 applied in `ι → E` to the diagonal subspace, and the transport needs `IsCompatiblePairing` for a pairing on a finite product (only the binary instance exists), `ri (univ.pi C) = univ.pi (ri C)` (~20 lines from `Convex.relint_iInter` and `Convex.relint_preimage`, both present), and — the real cost — `supportFn` of a `Set.pi` as a sum of `supportFn`s in `EReal`, with `⊤` terms to case-split and an ε-per-coordinate argument. Lift it with 4.4 and 9.18, which want the same tower | §16 | open — **blocking**, one §16 label |
...
| 11.17 | **`Caratheodory.lean` now splits a finite subset of `liftPD P D` twice** — inside `exists_linearIndepOn_of_mem_convexHull_add_coneHull` and standalone as `exists_finset_liftPD_eq`. Rebuilding the first on the second shortens the file by about 30 lines | §17 | open — friction the round created |
...
| 11.19 | **`posHomGen_mono` is declared twice, publicly, in the same namespace** — `HellyRefined.lean:196` and `Subgradient/Approx.lean:284`, the same statement proved twice. Neither module imports the other, so nothing is ambiguous *yet*; any module importing both would find the bare name unusable. It belongs once, in `Duality/Level.lean` where `posHomGen` is defined | sweep | open — pre-existing, and the only public collision in 4 751 declarations |
| 11.20 | **Four private near-duplicates across module boundaries**: `lift_mem_coneHull_liftPD` (private in `Caratheodory.lean`, public in `HullDirections.lean`, which imports it — legitimate, but worth a pointer), `iSup_sub_of_ne_bot` (`Bifunction/Algebra.lean`, `Optimization/Fenchel.lean`), `neg_coe_sub` (`Bifunction/Algebra.lean`, `Optimization/Adjoint.lean`, `Saddle/Existence.lean`, plus a primed variant in `Subgradient/Defs.lean`). Private, so harmless to the build; each is an `EReal` fact that should be a public lemma in `Tdaf/Order/EReal.lean` | sweep | open — cheap, and it shrinks four files |
