...
| 10.10 | **`Duality/Support.lean` has no `api.md` record.** Every other `Duality/*` module has one. Cost §13 a detour | §13 | **closed** — the record is written, and the two other modules in the same state went with it: `Recession/Function.lean` and `Eponyms.lean`. **Every module under `Tdaf/Analysis/` now has a record**, and unlike most claims in this ledger that one is mechanically checkable — walk the tree, collect `api.md`'s module headers, difference the two. The row's premise, that every other `Duality/*` module had one, was true; running the same check past `Duality/` is what found the other two |
...
| 11.20 | **Four private near-duplicates across module boundaries**: `lift_mem_coneHull_liftPD` (private in `Caratheodory.lean`, public in `HullDirections.lean`, which imports it — legitimate, but worth a pointer), `iSup_sub_of_ne_bot` (`Bifunction/Algebra.lean`, `Optimization/Fenchel.lean`), `neg_coe_sub` (`Bifunction/Algebra.lean`, `Optimization/Adjoint.lean`, `Saddle/Existence.lean`, plus a primed variant in `Subgradient/Defs.lean`). Private, so harmless to the build; each is an `EReal` fact that should be a public lemma in `Tdaf/Order/EReal.lean` | sweep | **the two `EReal` facts are done**; `lift_mem_coneHull_liftPD` is not. `Tdaf.EReal.iSup_sub_of_ne_bot` is the `Optimization/Fenchel.lean` variant, which is strictly stronger — `Bifunction/Algebra.lean` asked for `[Nonempty ι]` and over an empty index set both sides are `⊥`. `Tdaf.EReal.neg_coe_sub` is `-(↑r - z) = z - ↑r`. The primed variant is in **`Saddle/Defs.lean`**, not `Subgradient/Defs.lean`, and the sweep found a fifth spelling, `neg_coe_sub_eq` in `Optimization/Normal.lean`; both fold an `add_comm` into the statement and their call sites depend on the orientation, so they are now one-line aliases rather than deletions. **Now fully closed.** `lift_mem_coneHull_liftPD` is public in `Caratheodory.lean` and `HullDirections.lean` no longer re-proves it — closed as part of §12.33, which also showed that the copy to keep was the *layer-A* one, not the one the row assumed. And `Subgradient/Monotone.lean`'s `Tdaf.ConvexAnalysis.coe_sub_add_coe` — the same `EReal` fact as `Tdaf.EReal.coe_sub_add_coe`, under the same bare name in a different namespace — has moved to `Order/EReal.lean` as `Tdaf.EReal.coe_sub_add_coe′`, beside the twin it shadowed. The two are the same difference folded two ways (`(p - q) - u` against `(p - u) + (-q)`); a `rw` matches one and not the other, so both are kept, which is the same resolution the row records for the other orientation-dependent variants. Two namespaces sharing one bare name produce **no error** as long as the two are never in scope together — the same silence that let §12.34 stand for a round |

| 11.21 | **Two `Finset`-indexed relative-interior lemmas are parked above their home.** `Convex.relint_biInter_finset` (Theorem 6.5 over a `Finset`, in `Duality/Relint.lean`) and `Convex.relint_univ_pi` (in `Duality/FiniteProduct.lean`) both belong in `RelativeInterior.lean` beside `Convex.relint_iInter` and `intrinsicInterior_prod_eq`. They are where they are because that file has 19 direct importers and editing it costs a near-full rebuild. `RelativeInterior.lean` has Theorem 6.5 only for a `Type`-indexed family; the `Finset` form is what every `m`-ary statement wants | §16, §20 | **closed** — both are in `RelativeInterior.lean`, in a new `Indexed` section, with `univ_pi_eq_iInter_proj_preimage` brought along because it had no other consumer. **They could not go beside `Convex.relint_iInter` as the row says**: `Convex.relint_univ_pi` cites `Convex.relint_preimage`, which is declared 180 lines *below* it in the same file, so the section had to go at the end of §6 instead (LIB39 — the intra-file ordering blocker, hit for the second time this round). The row's worry about the 19 direct importers was the right worry about the wrong thing: the edit did force a near-full rebuild, but **not one call site changed**, because the move is *down* the DAG and every consumer already had the destination in its closure |
...
| 12.12 | **`Bifunction/Cofinite.lean` has one of the closing discussion’s three co-finiteness facts** (book 16693–16729), and its own `## What is not here` contradicts the paragraph §38 will need. Present: `cofiniteBifun_infConvBifun`, `adjointBifun_infConvBifun_of_cofinite`. Absent: that `Fλ`, `GF` and `F*` preserve co-finiteness (which needs the closedness-and-properness half of Theorem 38.3, and, for `GF`, `IsExactSum` over four spaces), and `dom F = ℝᵐ ∧ dom F* = ℝⁿ ⇒ co-finite`, which is Theorem 34.2 | §4.1 | **partly closed** with `Section38.lean` (report `surface/rockafellar-1970/reports/s38.md`). Added to `Bifunction/Cofinite.lean`: `smulRight_eq_conj_smul`, `closedFn_smulRight`, `proper_smulRight` (Theorem 38.3's closedness-and-properness half), `cofinite_smulRight`, `cofiniteBifun_smulRightBifun` (`Fλ`), and `CofiniteBifun.domConcaveBifun_adjointBifun_eq_univ`, `cofiniteBifun_of_domBifun_eq_univ`, `cofiniteBifun_iff_domBifun_eq_univ` (the `dom F = ℝᵐ ∧ dom F* = ℝⁿ` criterion, proved by Corollary 13.3.1 slice by slice rather than via Theorem 34.2). Still open: `cofiniteBifun_compBifun` for `GF`, and `F*` co-finite, which first needs a `CofiniteConcaveBifun` predicate — `lowerAdjointBifun` is the wrong reindexing. The module's `## What is not here` now matches reality |
...
| 12.27 | **Two more small §32 gaps.** `FinitelyGenerated.finite_extremePoints` → `Representation.lean` beside `extremePoints_convexHullPD_subset`: it is derived inline inside `exists_mem_extremePoints_isMaxOn_of_finitelyGenerated_of_bddAboveOnRays` and again on the surface, three times in all. `normalCone_innerₗ_closedBall` — the normal cone to the Euclidean unit ball at a unit vector, which is the whole content of the book's remark at line 14061 — → `Subgradient/Rademacher.lean` beside the inner-product bridges; proved on `Rn n` in ~30 lines via the equality case of Cauchy–Schwarz, and the proof is space-agnostic | §32 | **closed, both halves — one of them at a different home.** `normalCone_innerₗ_closedBall` is in `Subgradient/Rademacher.lean:389`, exactly where the row asked, and its section carries **neither `[FiniteDimensional]` nor completeness**, so it holds in any real inner-product space. `FinitelyGenerated.finite_extremePoints` could not go where the row said. That home is **unbuildable, for a failure mode not seen before**: `FinitelyGenerated` is defined in `Polyhedral/Defs.lean` and `finite_extremePoints_convexHullPD` in `Representation.lean`, and those two modules are **incomparable** in the import DAG — neither imports the other — so *neither* can host the lemma. Every previous instance of a bad home was "prerequisite above target", which moving one module fixes; this one has no such fix. It landed in `Polyhedral/Faces.lean`, which imports both, in the layer-A `Counting` section |
...
| 12.19 | **`autoImplicit` is still on in this repository.** `lakefile.toml` sets `relaxedAutoImplicit = false` but never `autoImplicit = false`, so a single-letter identifier in a theorem statement is silently auto-bound as an implicit with its type inferred from surrounding applications — and the file builds with **zero warnings**. Mathlib sets `autoImplicit = false`; this repository does not. One instance was found and fixed in `Part6/Section27.lean`; there is no telling how many others exist, because the symptom is nothing at all | §27 | **closed** — `autoImplicit = false` is set in `lakefile.toml`, and the tree builds clean: 2994 jobs, no errors, no warnings. **The six-module probe was a bad predictor.** It came back clean and suggested the flip was free; the full flip found a real instance the probe's six modules did not contain, in `RelativeInterior.lean` — a module none of them was. The instance is worth describing, because it is the shape to expect: `end Functions` closes the section declaring `variable {f : E → EReal}` **before** the two Theorem 7.5 statements that follow it, so `f` in both was auto-bound rather than taken from the section. The statements were *correct* — `ConvexFn f` pins the type — which is exactly why nothing ever complained: fifteen `Unknown identifier` errors appeared the moment the flag went off, and zero warnings before. Fixed by giving the pair its own `section SegmentLimits` with an explicit `f`, the idiom `section LevelSets` four lines below already uses. **One instance in 164 modules**, which is the useful number: the exposure was small, but it was not zero, and no amount of building would have found it |
...
| 12.35 | **`Tdaf/Analysis/Convex/Line.lean` is not registered in `Tdaf.lean`.** It has been in the tree since commit `f329515` and is reachable only because `Saddle/Differential.lean` imports it, so **every build is green and the aggregator never notices** — the one project invariant with no mechanical check behind it. Found by walking `Tdaf/` and differencing against `Tdaf.lean`'s import list | records pass | **closed** — registered, in alphabetical position between `Lattice` and `LinearInequalities`; 164 modules, 164 imports. The check is two lines of Python and should be run with the line-length check before every commit, since a build cannot catch it |
