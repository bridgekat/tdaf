...
**All of §9 is formalized.** Theorems 9.1–9.5 and Corollaries 9.1.1–9.1.3 are here; Corollary
9.2.1 and Corollary 9.2.2 are here too; Theorems 9.6–9.8 with Corollaries 9.6.1, 9.8.1 and 9.8.3
are in `Recession/ConeHull.lean`, which also supplies the *convex cone generated by a set* (it is
`PointedCone.hull ℝ`, so nothing new had to be defined) and the `cl K` formula of Theorem 8.2 that
`Recession/Cone.lean` deliberately skipped. Only the `m`-fold forms are deferred, and they are
*not* contentless inductions — the book runs Theorem 9.1 or 9.2 on an `m`-fold product;
Corollary 9.7.1 is left to §15 because it is a statement about gauges; Corollary 9.8.2 is
superseded by `Caratheodory.lean`.

**Theorem 8.2 in cone form needs no finite-dimensionality.** Rockafellar proves it through relative
interiors; the proof in `ConeHull.lean` is a direct sequential-closedness argument resting on the
single identity `x + a•p.2 − (a*p.1)•x ∈ C` for `0 ≤ a`, `a*p.1 ≤ 1`, which covers the `λ > 0`
branch and the recession branch uniformly. It is layer B; its three consumers are layer D only
because they route through Theorem 9.1. **Theorem 9.8 does not need Corollary 6.5.1** either: only
the easy inclusion of "slice of the closure = closure of the slice" is used, and the reverse comes
free from `closedConeOver_add_eq_closure_coe_sup`.
...
| `closedProperConvexFn_infConv_of_recessionFn_symm`, `exists_add_eq_of_infConv_le_of_recessionFn_symm`, `closedProperConvexFn_infConv`, `exists_add_eq_of_infConv_le` — **done** | **Cors 9.2.1 and 9.2.2**, formalized as `epi (f □ g) = epi f + epi g`, from which "the infimum is attained" falls out. Cor 9.2.1 for two functions, whose hypothesis — the directions of joint recession form a *symmetric* set — is genuinely weaker than Cor 9.2.2's; only the `m`-fold form is deferred |
...
| `closure_coe_hull`, `closure_coe_hull_eq_union`, `isClosed_coe_hull_of_isBounded` — **done** (`Recession/ConeHull.lean`) | **Thm 9.6**, Cor 9.6.1 |
| `posHomGen`, `epi_lscHull_posHomGen`, `lscHull_posHomGen`, `proper_posHomGen`, `lscHull_posHomGen_eq`, `posHomGen_eq_iInf_smulRight` — **done** | **Thm 9.7**; Cor 9.7.1 left to §15 (three lines from `lscHull_posHomGen_eq`) |
| `closure_convexHull_union_and_recessionCone`, `closure_convexHull_union`, `recessionCone_closure_convexHull_union`, `isClosed_convexHull_union_of_recessionCone_eq` — **done** | **Thm 9.8**, Cor 9.8.1; 9.8.2 superseded, 9.8.3 needs `convHullFn` for a family |
