...
| §26 | **Thms 26.1, 26.3, 26.4, 26.5 done, with Cors 26.3.1 and 26.4.1** (`Subgradient/Legendre.lean`, `EssentiallySmooth.lean`, `StrictlyConvex.lean`, `LegendreType.lean`): the Legendre conjugate is `f*` on `D = ∇f(C)`; `∂f` single-valued ⟺ essentially smooth; `f*` essentially smooth ⟺ `f` essentially strictly convex; `∂f` one-to-one ⟺ `f` of Legendre type; `D = dom ∂f*` so `ri (dom f*) ⊆ D ⊆ dom f*`; and `∇f : int (dom f) → int (dom f*)` is a bijection with `∇f* = (∇f)⁻¹`; and **Thm 26.6**, `∇f` a bijection of `E` onto itself ⟺ `f` strictly convex with `dom f* = E`. Left: Lemma 26.2, Cors 26.3.2–26.3.3, Lemma 26.7 |
