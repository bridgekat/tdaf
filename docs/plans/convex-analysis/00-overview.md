...
| §24 | **Thms 24.1–24.9 all done**, with Cor 24.5.1. Five modules: `Monotone.lean` (cyclic monotonicity — `IsCyclicallyMonotone`, `cyclicPotential`, and Thm 24.9 in both directions), `OneDim.lean` (`rightDeriv`/`leftDeriv`, and Thm 24.3 as "maximal monotone relations on `ℝ` = `∂f` of closed proper convex `f`"), `Convergence.lean` (Thms 24.5–24.6) and `Bounded.lean` (Thm 24.7). plus `Primitive.lean` (Thm 24.2's existence clause). **The plan was wrong about §24 three times.** Thm 24.9's uniqueness clause does not need 24.1–24.3 at all, and inverting that unlocked the rest; Thm 24.2's existence clause needs **no integral**, because the primitive is pinned down by its graph and Thm 24.3 produces it; and Thm 24.6's second assertion needs neither an `EReal`-valued Cor 10.8.1 (the points are eventually *interior*) nor a new Cor 23.5.3 (the library already had it). Missing: only Rockafellar's integral formula for the primitive, and Cor 24.2.1 |
