...
Tdaf/
  ...
  Analysis/Convex/
    ...
    Optimization/
      ...
      Adjoint.lean                    -- adjoint bifunctions, dual programs; §29 Thm 29.4, §30
      Normal.lean                     -- normality and the duality gap; §30 Thms 30.3-30.5
      Program.lean                    -- ordinary convex programs; §28 Thm 28.2
      Fenchel.lean                    -- Fenchel duality; §31 Thms 31.1, 31.3, 31.4
      ConeDuality.lean                -- §31 Cor 31.4.3, duality over a closed convex cone
    ...
    Bifunction/
      ...
      Cofinite.lean                   -- co-finite bifunctions; §38's closing remark
      Process.lean                    -- convex processes, both orientations; §39
      ProcessDuality.lean             -- the two inner products; §39 Thms 39.3, 39.4
...
| §8 | done (8.1–8.8) |
| §9 | **Complete** — Thms 9.1–9.5, Cors 9.1.1–9.1.3 and **Cors 9.2.1/9.2.2** (`Recession/Closedness.lean`), and Thms 9.6–9.8 with Cors 9.6.1, 9.8.1 and **9.8.3** (`Recession/ConeHull.lean`, which also carries Thm 8.2 in cone form and `posHomGen`). Everything is for two sets or two functions; the `m`-fold forms are deferred and are *not* contentless inductions. Cor 9.7.1 belongs to §15; Cor 9.8.2 is superseded by `Caratheodory.lean` |
...
the surface's `theorem_30_1_closed` is a one-line specialisation. **Nothing remains at Theorem 30.1**, and this
sentence used to name three things, all of which were wrong. The *properness* clause is a
biconditional (`properConcave_graphFn_adjointBifun_iff`); the surjectivity half of the one-to-one
correspondence is `theorem_30_1_surjective`; and the backbone never lacked a concave-side biadjoint
— `lowerAdjointBifun_lowerAdjointBifun_eq_clBifun` in `Bifunction/Algebra.lean`, read at `F = G⁎`,
*is* it, and its own docstring says so (remediation §12.24). §30 now has a declaration for every
numbered clause; only the *name* `clConcaveBifun` is missing. The stale text read: it
is
...
| `Saddle/Monotone.lean` | §37 — `partialInvertEquiv` (the **partial inversion** of Thm 37.5) with the transfer of maximal monotonicity across it, **Cor 37.5.1**'s homeomorphism clause `((u, y), (v, x)) ↦ (u - v, x + y)` and **Cor 37.5.2** (`ρ` is maximal monotone, including the differentiable clause `(u, v) ↦ (-∇₁K, ∇₂K)`) |
...
| `Bifunction/Algebra.lean` | §38 — the convex algebra: `infConvBifun` (`F₁ □ F₂`), `infConvFstBifun` (the same in the first variable), `smulRightBifun`, `imageBifun`, `compBifun`, `inverseBifun` (`F⫶`), `lowerAdjointBifun` (`F⫶*`), and the inner product `fenchelSup`/`fenchelInf`/`fenchelPairing`; **Thms 38.1–38.5 and 38.7**, **Lemma 38.6**, **Cors 38.2.1, 38.4.1, 38.5.1, 38.7.1** and the first equality of **Cor 38.7.2** |
| `Bifunction/Cofinite.lean` | §38's closing remark — `CofiniteBifun`, co-finiteness slice by slice through **Cor 13.3.1**, `cofinite_infConv`, `F₁ □ F₂` co-finite with `(F₁ □ F₂)* = F₁* □ F₂*` unconditionally, and the second equality of **Cor 38.7.2**; the one finite-dimensional module of §38 |
| `Bifunction/LinearProcess.lean` | §39 — linear transformations inside the convex algebra: the dictionary induced by `ConvexProcess.ofLinearMap`. Image, range and product go to the linear image, range and composite; `imageBifun_indicatorBifun_ofLinearMap` identifies `Af` with `mapLin A f` for `f` nowhere `⊥`; and `adjointProcess_ofLinearMap` / `coadjointProcess_ofLinearMap` identify the adjoint of a linear process with a transpose of the map, the **only** place in the bifunction and convex-process development where a transpose of a linear map occurs at all — everywhere else the adjoint is a defined operation of a pairing |
| `Bifunction/Process.lean` | §39 — `ConvexProcess` as a `PointedCone ℝ (U × X)` with `eval`, `dom`, `range`, `image`, `inv`, `comp`, `Add`, `indicatorBifun`, `adjointProcess`/`coadjointProcess`, `reflect` and `coBracket`; **Thms 39.1, 39.2, 39.5, 39.6, 39.8** with their closed halves, the open half of **Thm 39.7**, the first three assertions of **Thm 39.3**, **Cor 39.7.1**, and the infimum-oriented mirror of each |
| `Bifunction/ProcessDuality.lean` | §39 — the two inner products `⟨Au, x*⟩` and `⟨u, A* x*⟩` where they need a topology: **Thm 39.3**'s fourth and last assertions, **Thm 39.4**'s one-to-one correspondence between closed convex processes and their kernels, and the closed half of **Thm 39.7** |
...
| `Optimization/Adjoint.lean` | §30 — `adjointBifun` as the conjugate of the graph function at a reflected point, the concave adjoint, `clBifun`, all of **Thm 30.1** including `F** = cl F`, **Thm 30.2** (the dual objective is the *concave* conjugate of `-inf F`) and **Cor 30.2.2** (weak duality); also §29's **Thm 29.4** and **Cor 29.4.1**, which live here because `clBifun` does |
| `Optimization/Normal.lean` | §30 — `Normal` and its concave mirrors, **Cors 30.2.1–30.2.3**, **Thm 30.3** (normality of `(P)`, of `(P*)` and the absence of a duality gap are one condition), **Thm 30.4** in all ten clauses and **Thm 30.5** (the Kuhn–Tucker vectors of `(P)` are the optimal solutions of `(P*)`) |
| `Optimization/Program.lean` | §28 — ordinary convex programs: `feasibleSet`, `programLagrangian`, `optimalValue`, `IsKuhnTuckerVector`, and **Thm 28.2** under Slater's condition with Cors 28.2.1 and 28.2.2 |
