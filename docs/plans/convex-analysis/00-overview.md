...
| §27 | **Thms 27.1(a), 27.1(b), 27.2, 27.4 and the non-polyhedral case of Thm 27.3 done** (`Optimization/Minimum.lean`): `argmin`, `inf f = -f*(0)` (no hypotheses), the minimum set as `∂f*(0)`, existence, compactness and ε–δ well-posedness of the minimum set for a closed proper convex function with no direction of recession, Cors 27.2.1–27.2.2, minimisation over a closed convex set sharing no direction of recession with `f` (Cor 27.3.3 included), and the optimality condition `0 ∈ ∂h x + N_C(x)` in both halves. **Thm 27.1(f) is now done** (`recessionCone_setOf_le_eq_polarCone_dom_conj`), Thm 14.2 having arrived, and the first sentence of 27.1(i) with it (`zero_mem_closure_dom_conj_iff`, `Duality/Level.lean`); Thm 27.1(c), (d) and the rest of (i) need Thm 23.3 in full and Cor 13.3.4's remaining clauses, (g) and (h) need Thm 13.5, and (e) needs a reflexive pairing to be stated. **Cor 27.3.2 is done and does not need Helly** — `argmin_nonempty_of_polyhedralFn` reads it off the finitely generated epigraph, which is also what unblocks Thm 29.2's optimal-solution clause; Thm 27.3's polyhedral refinement and Cor 27.3.1 do still need Helly in the form of Thm 21.5 |
