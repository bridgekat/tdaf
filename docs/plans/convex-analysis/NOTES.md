...
550. **`rw` rewrites *every* occurrence of the instantiated pattern, so repeating the rewrite
...
551. **A bare `rw [map_sub]` or `rw [map_neg]` in a goal with nested applications hits the wrong
...
552. **`IsAdjointPair B B' ↑A ↑A'` will not `rw` into a goal that mentions `A x` for a
...
553. **Chain the rows of Theorem 12.3 with typed `have`s, not with `rw`.** Rewriting
...
554. **`biInf_le` does not exist in this Mathlib; the name is `iInf₂_le`.** And `iInf₂_le i hi`
...
555. **`simp` does not close the `⊥` and `⊤` branches of an `EReal` subtraction identity.** For
...
556. **`Tdaf.lean` is not actually sorted.** `Tdaf.Analysis.Convex.Optimization.Maximum` sits after
...
557. **The `unusedSectionVars` linter cascades: fix one `omit`, get two more warnings.** Adding
`omit [FiniteDimensional ℝ E] in` to a theorem changes which section variables the *next* theorems
are judged against, so the warning set has to be driven to a fixed point rather than fixed in one
pass. Iterate `lean_diagnostic_messages` on the file — four rounds was normal for
`Optimization/ConeDuality.lean` — and expect the reported line numbers to move each round, since
every `omit` inserts a line.
