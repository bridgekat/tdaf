...
400. **`omit [inst] in` goes before the doc comment, not between it and `theorem`.** Putting it
...
401. **`IsCompatiblePairing ((innerₗ E).flip)` is as invisible to instance search as the continuous
...
402. **`simp` turns `⟪y, y⟫_ℝ = 0` into `0 = ‖y‖ ^ 2`, with the sides swapped.** So
...
403. **A hypothesis `h : x ∈ {z | P z}` must be re-ascribed before `linarith` sees `P x`.**
...
404. **Do not `rw [Function.comp_def]` to line up a composed sequence; use `Tendsto.congr`.**
...
405. **`Metric.isBounded_range_of_tendsto` bounds the *whole* range of a convergent sequence.**
