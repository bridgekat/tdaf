...
404. **A product of inner-product spaces blocks every §31 result at `prodPairing`.** This is
...
403. **`dif_pos` is deprecated (→ `dite_eq_left`), which kills the `dite`-plus-`Exists.choose`
...
402. **`closedFn_conj` wants `IsContinuousPairing B.flip`, which is not an instance for
...
401. **`abel` emits an `info` "Try this: abel_nf" when normalisation alone closes the goal.** It
...
400. **The Bash tool truncates a command at roughly 8 KB, which silently breaks heredocs.** Two
