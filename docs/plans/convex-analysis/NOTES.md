...
420. **A worktree's `.lake/build` can be stale while `lake` reports it up to date.** The symptom is
...
421. **`EReal.bot_add_of_ne_bot` does not exist, although `EReal.top_add_of_ne_bot` does.** For the
...
422. **`rw [EReal.neg_add h₁ h₂]` takes its implicit `a` and `b` from the *types of the side
...
423. **`lowerAdjointBifun A B H v y` is *defined* as `-(adjointBifun A B H y v)`, so a goal
...
424. **`map_smul` does not reach a scalar sitting in front of an *applied* linear map.** After
...
425. **`lake` 5.0.0 has no `-j` / `--jobs` option**, so the job count cannot be capped from the
