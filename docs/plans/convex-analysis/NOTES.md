...
600. **`if_pos` and `if_neg` are deprecated** (use `ite_eq_left` / `ite_eq_right`), and a statement
...
601. **`Set.mem_setOf_eq` is deprecated in favour of `Set.mem_ofPred_eq`, and `push_neg` in favour
...
602. **Membership in a pointwise `s + t` of sets unfolds with the equation the *other* way round.**
...
603. **`EReal.eq_bot_iff_forall_lt` and `EReal.eq_top_iff_forall_lt` take the `EReal` explicitly.**
...
604. **`PosHomogeneous.epiCone` gives membership that is defeq to `epi`, but `coe_epiCone` will not
...
605. **`Sum.noConfusion h` does not elaborate from `h : Sum.inr p = Sum.inl j`** — it reports an
...
606. **A theorem whose only occurrence of the pairing `B` is in a typeclass argument cannot infer
...
607. **Editing a repository file with Python's text-mode open-for-write on Windows rewrites every
