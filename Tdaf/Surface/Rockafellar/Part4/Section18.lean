...
* `facesEquivFacesInterOrthogonal` — the book's one-to-one correspondence (p. 166) between the
  faces of `C` and the faces of `C ∩ L^⊥`, as an `Equiv` between the two subtypes.
  `eq_add_inter_orthogonal_of_isFace` is the formula `C' = C₀' + L` on its own.
...
* **The face correspondence of p. 166 needs no hypothesis on `C` at all.** The book introduces it
  for a closed convex set of non-zero lineality; the backbone's `isFaceEquivInter` is stated for
  an arbitrary subspace `N` of the lineality space and an arbitrary complement `M`, with no
  closedness, no convexity of `C` and no finite dimension. `L` and `L^⊥` are one instance.
...
/-- **Rockafellar, §18 (p. 166).** A face of `C` splits as `C' = C₀' + L`, where `L` is the
lineality space of `C` and `C₀' = C' ∩ L^⊥`.

A face absorbs the lineality of the set it is a face of (`IsFace.linealitySpace_subset`), so the
decomposition `C = L + (C ∩ L^⊥)` of the remark just above applies to `C'` itself. -/
...
/-- **Rockafellar, §18 (p. 166).** With `L` the lineality space of `C` and `C₀ = C ∩ L^⊥`, the
faces `C'` of `C` are in one-to-one correspondence with the faces `C₀'` of `C₀`, by the formulas
`C' = C₀' + L` and `C₀' = C' ∩ L^⊥`.

The book asserts this ("evidently") without proof. The forward map is `IsFace.inter_convex`, the
backward map is `isFace_add_of_isFace_inter`, and the two round trips are
`eq_add_inter_orthogonal_of_isFace` and `inter_add_eq_self_of_isCompl`. None of it needs `C`
closed, convex, or finite-dimensional; only that `L^⊥` is a complement of `L`. -/
