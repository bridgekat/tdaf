...
| p. 202, the interval reading | `interval_reading_le`, `interval_reading_nonneg_eq` |
...
* **The subspace `L`** (p. 202): `intervalSubspace A` is the graph of `A` written in `ℝᴺ`,
  `N = n + m`, the `n`-dimensional subspace whose points are the vectors
  `(ξ₁, …, ξₙ, (Ax)₁, …, (Ax)ₘ)`; `intervalVector` is the map that produces them, and
  `interval_reading_le` / `interval_reading_nonneg_eq` are the two readings of p. 202. The
  identification of `ℝⁿ × ℝᵐ` with `ℝᴺ` is `euclideanProdEquiv`
...
The identification of `ℝⁿ × ℝᵐ` with `ℝᴺ` is `euclideanProdEquiv`
(`Tdaf/Analysis/Convex/EuclideanProd.lean`). -/

/-- The vector `z = (ξ₁, …, ξ_n, (Ax)₁, …, (Ax)_m)` of `ℝᴺ`, `N = n + m`, that a solution `x` of a
system with `m` constraints becomes when the constraint values are appended to the unknowns
(p. 202). -/
...
/-- **The subspace `L` of p. 202**: the vectors `z ∈ ℝᴺ` satisfying `ζ_{n+i} = ∑ⱼ αᵢⱼζⱼ`, that is,
the graph of `A` written in `ℝᴺ`. Rockafellar calls it "a certain `n`-dimensional subspace of
`ℝᴺ`", and it is the object Theorem 22.6 is stated about, in place of the coefficient matrix. -/
...
/-- **Rockafellar, p. 202, the first interval reading**: the system `Ax ≤ a` in `ℝⁿ` has a solution
exactly when the subspace `L` meets the generalized rectangle cut out by `Iⱼ = (-∞, +∞)` for
`j ≤ n` and `I_{n+i} = (-∞, αᵢ]`.

This is the correspondence the whole second half of §22 is stated over; the theorem of the
alternative for it is `theorem_22_1_matrix`, and the conjectured general alternative — for an
arbitrary subspace and arbitrary intervals — is Theorem 22.6, deferred. -/
