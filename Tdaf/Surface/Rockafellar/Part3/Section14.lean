...
Ten of the eleven numbered results are here, stated over `Rn n = ℝⁿ` and closed by specialising
`Tdaf/Analysis/Convex/Duality/{Polar, PolarBounded, Gauge, Level, Barrier}.lean` and
...
* **Theorem 14.4** (the cone in `ℝⁿ⁺²` generated by the `(1, x, μ)` with `μ ≥ f(x)`, whose polar
  carries `f*`) — *deferred by scope*, with the prerequisites corrected. It does **not** need the
  recession function of the positively homogeneous function generated by `f`, which is what
  `Duality/Polar.lean` used to defer it for: that description of `cl K` is the unnumbered paragraph
  *before* the theorem and the printed proof never uses it. It needs the `ℝ × ℝⁿ × ℝ ≃ ℝⁿ⁺²`
  transport — `Tdaf/Analysis/Convex/EuclideanProd.lean` supplies `ℝᵐ × ℝⁿ ≃ ℝᵐ⁺ⁿ` and what
  remains is the one-dimensional factor — and the density step of Corollary 6.5.2. See that
  module's deferral note for the full list.
