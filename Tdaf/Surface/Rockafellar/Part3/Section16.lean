...
`Lᗮ` *is* the annihilator of `L` for the pairing: `Submodule.mem_orthogonal` is `∀ u ∈ L, ⟨u, x*⟩ =
0` definitionally, and so is `pairing_apply`. Properness of `f*` is discharged by
`proper_conj_of_proper`, which is Theorem 12.2 read in finite dimensions. -/
...
/-- **Corollary 16.2.1.** Let `A` be a linear transformation from `ℝⁿ` to `ℝᵐ` and let
...
/-- **Theorem 16.3**, the exact half: if there is an `x` with `Ax ∈ ri (dom g)`, the
...
/-- **Theorem 16.3**, the attainment: under the same qualification, for each `x*` the
infimum `inf {g*(y*) | A*y* = x*}` is attained (or is `+∞` vacuously).

The backbone's guard `< ⊤` is exactly the book's "or is `+∞` vacuously". -/
...
/-- **Corollary 16.3.1**, first formula: `δ*(y* | AC) = δ*(A*y* | C)` for any convex
set `C` in `ℝⁿ`.
...
/-- **Corollary 16.3.2**, first formula: `(AC)° = A*⁻¹(C°)` for any convex set `C`
in `ℝⁿ`.
...
/-- **Theorem 16.4**, second formula: `(cl f + cl g)* = cl(f* □ g*)`.
...
/-- **Theorem 16.4**, the exact half: if `ri (dom f)` and `ri (dom g)` have a point in
common, the closure operation can be omitted and `(f + g)* = f* □ g*`.

Closedness is not assumed, as in the book. -/
...
/-- **Corollary 16.4.1**, second formula:
...
The `□`-product is the `AddCommMonoid` sum of `InfConvFn`, exactly as in
`theorem_16_4_infConv_finset`; the family is indexed by a `Finset` rather than by `Fin m`, so `m =
0` is the (vacuous) empty family and the book's `m ≥ 1` is `hs`. -/
...
/-- **Theorem 16.5**, first formula: `(conv {fᵢ | i ∈ I})* = sup {fᵢ* | i ∈ I}` for an
