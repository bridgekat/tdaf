...
**Theorem 23.9's two constructors are both built here.** The backbone's `IsExactImage.of_relint`
asks for `ClosedProperConvexFn h`, while the book asks only for `h` proper convex; §16 already pays
the reduction to the closed case inside `theorem_16_3_exact` and `theorem_16_3_attained`, and
`theorem_23_9` reads the interface back off those. The polyhedral clause has no backbone
constructor at all and is built from Theorem 19.2 and Corollary 19.3.1 in
`isExactImage_of_polyhedral`. Both are recorded under **Backbone gaps**.
...
4. **`IsExactImage.of_polyhedral`**, in `Tdaf/Analysis/Convex/Polyhedral/Duality.lean`. Wanted:
   `PolyhedralFn g → Proper g → A x₀ ∈ dom g → IsExactImage B B' A A' hA g` — Theorem 20.1's
   companion for the *image* rule, the polyhedral strengthening of Theorem 16.3. That file has the
   two sum constructors (`IsExactSum.of_polyhedral`, `IsExactFinsetSum.of_polyhedral`) and no
   image one, so the last clause of both Theorem 16.3 and Theorem 23.9 is unreachable from the
   backbone. `isExactImage_of_polyhedral` below is the proof, and it is entirely §19's: `g*` is
   polyhedral (Theorem 19.2), `A*g*` is polyhedral with its infimum attained
   (Corollary 19.3.1), and a polyhedral proper function is closed, so Theorem 16.3's closure
   formula has nothing left to close. **§16 should take this**, not §23: its
   `theorem_16_3_exact` has the same missing clause.
5. **`indicatorFn_finsetSum`**, in `Tdaf/Analysis/Convex/Indicator.lean`. Wanted:
...
/-- **Theorem 23.9's polyhedral constructor.** For a *polyhedral* proper `g` it is enough that the
range of `A` meets `dom g`, with no relative interior.

This is the missing `IsExactImage.of_polyhedral`: `Polyhedral/Duality.lean` supplies only the sum
constructors. The route is Rockafellar's own — Theorem 19.2 makes `g*` polyhedral, Corollary 19.3.1
makes `A*g*` polyhedral (hence closed, and with its infimum attained), and Theorem 16.3's *closure*
formula then has nothing left to close. -/
private theorem isExactImage_of_polyhedral (A : Rn n →ₗ[ℝ] Rn m) {g : Rn m → EReal}
    (hg : PolyhedralFn g) (hp : Proper g) {x₀ : Rn n} (hx₀ : A x₀ ∈ dom g) :
    IsExactImage (pairing n) (pairing m) A (LinearMap.adjoint A)
      (isAdjointPair_adjoint A) g := by
  have hgcl : ClosedFn g := hg.closedFn hp.ne_bot
  have hconjpoly : PolyhedralFn (conj (pairing m) g) := PolyhedralFn.conj hg
  have hmappoly : PolyhedralFn (mapLin (LinearMap.adjoint A) (conj (pairing m) g)) :=
    polyhedralFn_mapLin hconjpoly _
  have hne : (dom (compLin g A)).Nonempty := ⟨x₀, by rwa [mem_dom, compLin_apply]⟩
  have hbot : ∀ y, conj (pairing n) (compLin g A) y ≠ ⊥ := fun y => conj_ne_bot hne y
  have hmapbot : ∀ y, mapLin (LinearMap.adjoint A) (conj (pairing m) g) y ≠ ⊥ := by
    intro y hy
    exact hbot y
      (le_bot_iff.1 (hy ▸ conj_compLin_le_mapLin (isAdjointPair_adjoint A) g y))
  have hmapcl : ClosedFn (mapLin (LinearMap.adjoint A) (conj (pairing m) g)) :=
    hmappoly.closedFn hmapbot
  have heq : conj (pairing n) (compLin g A)
      = mapLin (LinearMap.adjoint A) (conj (pairing m) g) := by
    have h := theorem_16_3_closure A hg.convexFn
    rwa [show clFn g = g from hgcl,
      show clFn (mapLin (LinearMap.adjoint A) (conj (pairing m) g))
        = mapLin (LinearMap.adjoint A) (conj (pairing m) g) from hmapcl] at h
...
from Theorem 16.3 via Corollary 19.3.1", which is exactly the route
`isExactImage_of_polyhedral` takes. -/
theorem theorem_23_9_polyhedral (A : Rn n →ₗ[ℝ] Rn m) {h : Rn m → EReal} (hh : PolyhedralFn h)
    (hp : Proper h) {x₀ : Rn n} (hx₀ : A x₀ ∈ dom h) (x : Rn n) :
    subgradient (pairing n) (compLin h A) x
      = LinearMap.adjoint A '' subgradient (pairing m) h (A x) :=
  (isExactImage_of_polyhedral A hh hp hx₀).subgradient_compLin x
