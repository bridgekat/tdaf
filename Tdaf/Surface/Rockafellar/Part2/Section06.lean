...
`D = {y | C_y ≠ ∅}`, `(y, z) ∈ ri C` iff `y ∈ ri D` and `z ∈ ri C_y`. `D` is written in the
equivalent form `Prod.fst '' C`. -/
...
`{(1, x) | x ∈ C}`, `ri K` is the set of `(λ, x)` with `λ > 0` and `x ∈ λ (ri C)`. Stated about
`K ∪ {0}`, which has the same closure as `K`. -/
