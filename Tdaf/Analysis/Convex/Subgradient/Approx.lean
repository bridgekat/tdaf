...
convex and decreases to `∂f x` as `ε ↓ 0`; **Theorem 23.6** says its support function decreases to
the *directional derivative* `f'(x; ·)`, not merely to the closure `δ*(· | ∂f x)`. That is the
exact dual of the discrepancy between `f'(x; ·)` and its own closure.
...
* `posHomGen_shiftFn` — the directional derivative *is* the positively homogeneous convex function
  generated by the shift. This is the identity Theorem 23.6 turns on, and it needs no topology.
...
The book's `lim_{ε ↓ 0}` is an infimum over `ε > 0` here, the same thing because `ε ↦ ∂_ε f x` is
monotone. Finite dimension is used once, through Theorem 9.7, which makes the generated function
closed and so removes the closure from Theorem 13.5. `shiftFn` adds a real constant rather than
subtracting `f x`, which avoids `EReal` subtraction; every lemma below carries `f x = (r : ℝ)`.

## References

* R. T. Rockafellar, *Convex Analysis*, Princeton University Press, 1970, §23 (Theorem 23.6).
-/
