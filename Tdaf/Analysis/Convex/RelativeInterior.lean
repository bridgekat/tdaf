...
* `Convex.subset_closure_inter_setOf_pos`, `Convex.closure_inter_setOf_pos` — a convex set on
  which a linear functional is non-negative and somewhere positive is the closure of the part where
  the functional is positive. A segment argument in a topological vector space, with no relative
  interiors in it; it is here because it is the density step of the cross-section arguments.
...
* `Convex.relint_cone_prodMk_one` — Corollary 6.8.1, the relative interior of the convex cone
  generated by `{1} × C`, with `cone_prodMk_one_convexHull_union` (Theorem 3.8 for two sets) and
  `Convex.relint_convexHull_union` (Theorem 6.9 for two sets).
...
* `ConvexFn.relint_setOf_le`, `ConvexFn.closure_setOf_le` — Theorem 7.6, the relative interior
  ...
  `ConvexFn.relint_setOf_lt_eq` and `ConvexFn.closure_setOf_lt_eq` (the strict level set has
  the same two) and Corollary 7.6.1.
...
* Theorem 7.6, the `ri`-flavoured half (deferred by `Tdaf.Analysis.Convex.Closure`, whose
  `lscHull_le_setOf` is the half that does not need relative interiors) —
  `ConvexFn.relint_setOf_le` and `ConvexFn.closure_setOf_le`.
...
`x + t (x₀ - x) = (1 - t) x + t x₀` have `ℓ` value `(1 - t) ℓ x + t ℓ x₀ > 0` for `0 < t ≤ 1`, and
they converge to `x` as `t` decreases to `0`.

No continuity of `ℓ` is needed: the approximating points are produced explicitly, so the set
`{x | 0 < ℓ x}` never has to be open. -/
...
explicit set `insert 0 {p | 0 < p.1 ∧ p.2 ∈ p.1 • C}`;
`Tdaf/Analysis/Convex/Recession/ConeHull.lean`'s `coe_hull_prodMk_one` is the identification of
that set with `PointedCone.hull ℝ ({1} × C)`, and `closedConeOver` there is its closure. -/
...
omit [FiniteDimensional ℝ E] in
/-- The cone `{(λ, x) | λ > 0, x ∈ λC} ∪ {0}` over a convex set is convex.

By Theorem 2.6 it would be enough to check closure under addition, but that lemma lives in a
module this file does not import; the convex combination is checked directly, and
`Convex.add_smul` (`(s + t) • C = s • C + t • C`) is what carries the only interesting case. -/
...
`{1} × C` consists of the pairs `(λ, x)` with `λ > 0` and `x ∈ λ (ri C)`.

This is Theorem 6.8 with the first factor taken to be `ℝ`: the projection of the cone is
`[0, ∞)`, whose relative interior is `(0, ∞)`, and the slice above `λ > 0` is `λ C`, whose
relative interior is `λ (ri C)` by Corollary 6.6.1. -/
...
is the union of the relative interiors of the convex combinations.

Rockafellar's proof: pass to the cones one dimension up, where the convex hull of the union becomes
a *sum* (Theorem 3.8), apply Corollary 6.6.2 to the sum, and read the answer off with
Corollary 6.8.1. Only the two-set case is stated; the step to `m` sets is a bare induction, and the
file's precedent for that is `Tdaf/Analysis/Convex/Recession/ConeHull.lean`'s Theorem 9.8. -/
...
is already below that level at some *relative interior* point of its effective domain.

Rockafellar's proof verbatim: the open half-space `{(x, μ) | μ < α}` meets `epi f`, hence meets
`ri (epi f)` by Corollary 6.3.2, and Lemma 7.3 reads off the point. -/
...
`E × (-∞, α]`, whose relative interior is `E × (-∞, α)`: the level set is the projection of
`epi f ∩ slab`, Theorem 6.5 splits the relative interior of that intersection, and Theorem 6.6
pushes it through the projection. Properness is not needed — see the module note. -/
...
stated for the lower semicontinuous hull, which is what carries the content.

`⊆` is `closure_le_subset_lscHull_le` and needs no hypothesis. For `⊇`, Corollary 7.3.1 supplies a
point `y` of `ri (dom f)` with `f y < α`, so `(y, α) ∈ ri (epi f)` while `(x, α) ∈ cl (epi f)`;
Theorem 6.1 runs the segment between them inside `ri (epi f)`, and its points project to points of
`{z | f z < α}` converging to `x`. -/
...
/-- **Rockafellar, Corollary 7.6.1**, first formula: for a convex function whose effective domain
is relatively open, the relative interior of a level set is the corresponding strict level set.
...
closure of a strict level set is the corresponding level set.

Rockafellar assumes in addition that `dom f` is relatively open; that hypothesis is needed only
