...
* **Theorem 14.4** (the `ℝ^(n+2)` cone whose polar carries `f*`) needs the recession function of
  the positively homogeneous function generated by `f` — a part of Theorem 13.5 that the book does
  not print, and that neither of the two printed assertions (`clFn_posHomGen`,
  `supportFn_setOf_le_zero` in `Tdaf/Analysis/Convex/Duality/Level.lean`) supplies — together with
  the transport between `ℝⁿ × ℝ × ℝ` and `ℝⁿ⁺²`.
