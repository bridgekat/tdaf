...
* `polarCone_coe_hull` — a polar cone sees only the cone a set generates.
...
27.1(f) consumes; `polarCone_coe_hull` recovers the book's phrasing, since a polar cone cannot tell
a set from the cone it generates.
...
/-- **A polar cone sees only the cone a set generates.** -/
theorem polarCone_coe_hull (B : E →ₗ[ℝ] F →ₗ[ℝ] ℝ) (S : Set E) :
...
theorem recessionConeFn_conj (hp : Proper f) (hc : Proper (conj B f)) :
  ...
  ext y
  rw [mem_recessionConeFn, recessionFn_conj hp hc, ← _root_.EReal.coe_zero, supportFn_le_coe_iff]
  exact Iff.rfl
...
theorem recessionConeFn_conj_hull (hp : Proper f) (hc : Proper (conj B f)) :
  ...
  rw [polarCone_coe_hull, recessionConeFn_conj hp hc]
...
theorem polarCone_recessionConeFn (hf : ConvexFn f) (hcl : ClosedFn f) (hp : Proper f) :
  ...
  rw [recessionConeFn_eq_polarCone_dom_conj (B := B) hf hcl hp, ← polarCone_coe_hull B.flip,
    ← hK]
...
theorem zero_mem_interior_iff_polarCone_eq_zero {D : Set F} (hD : Convex ℝ D) (hne : D.Nonempty) :
  ...
  · intro hpol
    have hbip : closure ((PointedCone.hull ℝ D : PointedCone ℝ F) : Set F) = Set.univ := by
      ...
      rw [polarCone_coe_hull, hpol] at h
      rw [← h]
      ext x'
      simp [polarCone]
