...
* **Corollary 19.1.2 for functions** — that `k₀`, the positively homogeneous convex function
  generated by the conjugates of finitely many affine functions, is polyhedral. Its epigraph is the
  convex cone generated by the finitely many points `(aᵢ, αᵢ)` together with `(0, 1)`, so
  Minkowski–Weyl (`Polyhedral/Cone.lean`) should give it, but the identification of
  `posHomGenCone (convFn …)` with that finitely generated cone does not exist.
...
Three smaller pieces are also missing: the epigraph-sum description of `conv {k₀, k₁}`; the
polyhedrality of the positively homogeneous function generated by finitely many affine functions;
and the subadditivity step `posHomGen h (x + y) ≤ posHomGen f x + posHomGen g y` for `h ≤ f, g` with
