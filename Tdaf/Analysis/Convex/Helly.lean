...
**Theorems 21.4 and 21.5 are not formalised.** They weaken Theorem 21.3's recession hypothesis to
...
Rockafellar's proof splits `k` as `conv {k₀, k₁}`, identifies `epi k₀ + epi k₁` with `epi k`
(Theorem 3.8), reads `k(0)` as an infimal convolution, and then needs `k₀` polyhedral
(Corollary 19.1.2 for *functions* generated by finitely many points plus the upward direction),
Theorem 20.1 to conjugate `g + k₁`, and the identification of `kⱼ*` with `δ(· | Cⱼ)`.

**The Theorem 20.1 step is no longer a gap.** `k₁` is a positively homogeneous *generated*
function, so it need be neither closed nor proper, and earlier versions of
`IsExactSum.of_polyhedral` asked for `ClosedProperConvexFn` on the non-polyhedral summand where the
book asks only for proper convex. `IsExactSum.of_polyhedral` and `IsExactSum.of_relint` now carry
the book's hypotheses; the removal is **Theorem 9.3** in the conjugate form
`conj_add_eq_conj_clFn_add_clFn`
(`Duality/Relint.lean`; the book's own form of Theorem 9.3, `clFn_add`, was already in
`Recession/Closedness.lean` and is too strong to use here). What remains for `k₁` is only the case
distinction the book itself makes: `k₁` improper is handled separately, before Theorem 20.1 is
invoked at all.

**Corollary 19.1.2 for functions is no longer a gap.** `polyhedralFn_posHomGen_of_epi_eq`
(`Polyhedral/Homogeneous.lean`) proves that `posHomGen f` is polyhedral whenever `epi f` is the
convex hull of finitely many points plus the vertical ray `cone {(0,1)}`, and computes its epigraph
as the cone generated by those points together with `(0, 1)` — Rockafellar's generator list, extra
point included. The hypothesis is the vertical-ray one and not "`f` is polyhedral", because with a
general direction set the identity is false; the vertical-ray case is the one §21 needs, since the
conjugate of an affine function is a point indicator.
...
Note that the epigraph-sum description of `conv {k₀, k₁}` is needed only in the direction
`k 0 ≤ k₀ (-z) + k₁ z`, which is `posHomGen_mono` plus the fact that `epi k` is a convex cone
containing both `epi k₀` and `epi k₁`; the full identity `k = k₀ □ k₁` is not required.

Two smaller pieces are also missing: the epigraph-sum description of `conv {k₀, k₁}`; and the
subadditivity step `posHomGen h (x + y) ≤ posHomGen f x + posHomGen g y` for `h ≤ f, g` with
both values `≠ +∞`, which `PosHomogeneous.convexFn_iff_subadditive` does *not* supply here because
it needs `∀ x, f x ≠ -∞` — exactly what an improper `k₁` violates.
...
theorem alternative_infinite_system_univ [IsCompatiblePairing B] [IsCompatiblePairing B.flip]
  ...
  have hk0 : posHomGen (convFn fun i => conj B (f i)) (0 : F) = ⊥ := by
