...
Theorem 20.1 to conjugate `g + k₁`, and the identification of `kⱼ*` with `δ(· | Cⱼ)`. Of these the
missing pieces are the epigraph-sum description of `conv {k₀, k₁}` and the polyhedrality of the
positively homogeneous function generated by finitely many affine functions; the rest is available.
