...
The load-bearing gap is the last one but one. `k₁` is a positively homogeneous *generated* function:
it need be neither closed nor proper. The book's Theorem 20.1 asks only that the summands be proper
convex, but `IsExactSum.of_polyhedral` asks for `ClosedProperConvexFn` on the non-polyhedral
summand, and it inherits that from `IsExactSum.of_relint` (Theorem 16.4), whose proof runs through
`conj_add_eq_clFn_infConv`, where closedness is load-bearing — replacing `f` and `g` by their
closures moves the inequality the wrong way. Closing the gap means proving
`cl (f + g) = cl (cl f + cl g)` when `ri (dom f) ∩ ri (dom g) ≠ ∅` and threading it through
Theorem 16.4, which is a §16 project, not a §21 one.
