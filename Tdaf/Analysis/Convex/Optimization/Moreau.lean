...
Rockafellar's Theorem 31.5. In a real Hilbert space paired with itself by the inner product, the
quadratic `w z = ½‖z‖²` is its own conjugate, and infimal convolution with it splits `w` between
a closed proper convex function and its conjugate:

`(f □ w) + (f* □ w) = w`.

## Main results

* `quadFn` — Rockafellar's `w z = ½|z|²` as an `EReal`-valued function.
* `conj_quadFn` — the quadratic is self-conjugate under the inner-product pairing.
* `conj_quadFn_sub` — `(w(z - ·))*(y) = ⟨z, y⟩ + w y`, the translate computation the theorem runs
  on.
...
/-- **Rockafellar's `w`**: `w z = ½|z|²`, as an `EReal`-valued function so that it lives in the
same world as `conj` and `infConv`. -/
noncomputable def quadFn (E : Type*) [NormedAddCommGroup E] : E → EReal :=
  fun z => ((‖z‖ ^ 2 / 2 : ℝ) : EReal)

omit [InnerProductSpace ℝ E] in
theorem quadFn_apply (z : E) : quadFn E z = ((‖z‖ ^ 2 / 2 : ℝ) : EReal) := rfl

omit [InnerProductSpace ℝ E] in
theorem quadFn_ne_bot (z : E) : quadFn E z ≠ ⊥ := _root_.EReal.coe_ne_bot _

omit [InnerProductSpace ℝ E] in
theorem quadFn_ne_top (z : E) : quadFn E z ≠ ⊤ := _root_.EReal.coe_ne_top _

omit [InnerProductSpace ℝ E] in
@[simp] theorem quadFn_neg (z : E) : quadFn E (-z) = quadFn E z := by
  rw [quadFn_apply, quadFn_apply, norm_neg]

/-- The inequality behind convexity of `½‖·‖²`: the defect is `ab‖u - v‖² ≥ 0`. -/
private theorem norm_combo_sq_le {u v : E} {a b : ℝ} (ha : 0 ≤ a) (hb : 0 ≤ b) (hab : a + b = 1) :
    ‖a • u + b • v‖ ^ 2 / 2 ≤ a * (‖u‖ ^ 2 / 2) + b * (‖v‖ ^ 2 / 2) := by
  obtain rfl : b = 1 - a := by linarith
  have hnu : ‖a • u‖ = a * ‖u‖ := by rw [norm_smul, Real.norm_eq_abs, abs_of_nonneg ha]
  have hnv : ‖(1 - a) • v‖ = (1 - a) * ‖v‖ := by
    rw [norm_smul, Real.norm_eq_abs, abs_of_nonneg hb]
  have hip : ⟪a • u, (1 - a) • v⟫ = a * ((1 - a) * ⟪u, v⟫) := by
    rw [real_inner_smul_left, real_inner_smul_right]
  have hexp : ‖a • u + (1 - a) • v‖ ^ 2
      = (a * ‖u‖) ^ 2 + 2 * (a * ((1 - a) * ⟪u, v⟫)) + ((1 - a) * ‖v‖) ^ 2 := by
    rw [norm_add_sq_real, hnu, hnv, hip]
  have hkey : 0 ≤ a * ((1 - a) * (‖u‖ ^ 2 - 2 * ⟪u, v⟫ + ‖v‖ ^ 2)) := by
    rw [← norm_sub_sq_real u v]
    exact mul_nonneg ha (mul_nonneg hb (sq_nonneg _))
  nlinarith [hexp, hkey]

/-- `x ↦ w(z - x)` is a convex function. -/
theorem convexFn_quadFn_sub (z : E) : ConvexFn (fun x => quadFn E (z - x)) := by
  ...
  refine le_trans (norm_combo_sq_le ha hb hab) ?_
...
theorem proper_quadFn_sub (z : E) : Proper (fun x => quadFn E (z - x)) :=
...
theorem continuous_quadFn_sub (z : E) : Continuous (fun x => quadFn E (z - x)) := by
  have h : Continuous fun x : E => (‖z - x‖ ^ 2 / 2 : ℝ) := by fun_prop
  exact continuous_coe_real_ereal.comp h

/-- **The quadratic is self-conjugate** under the inner-product pairing: `w* = w`. The supremum
`⨆ x (⟨x, y⟩ - ½‖x‖²)` has defect `½‖x - y‖²` and is attained at `x = y`. -/
theorem conj_quadFn (y : E) : conj (innerₗ E) (quadFn E) y = quadFn E y := by
  rw [conj_apply]
  refine le_antisymm (iSup_le fun x => ?_) ?_
  · rw [quadFn_apply, quadFn_apply, ← _root_.EReal.coe_sub, _root_.EReal.coe_le_coe_iff,
      innerₗ_apply_apply]
    nlinarith [norm_sub_sq_real x y, sq_nonneg ‖x - y‖]
  · refine le_trans (le_of_eq ?_)
      (le_iSup (fun x : E => ((innerₗ E x y : ℝ) : EReal) - quadFn E x) y)
    rw [quadFn_apply, ← _root_.EReal.coe_sub, _root_.EReal.coe_eq_coe_iff,
      innerₗ_apply_apply, real_inner_self_eq_norm_sq]
    ring

/-- **The conjugate of a translate of the quadratic**: `(w(z - ·))*(y) = ⟨z, y⟩ + w y`. The
supremum has defect `½‖(x - z) - y‖²` and is attained at `x = z + y`. -/
theorem conj_quadFn_sub (z y : E) :
    conj (innerₗ E) (fun x => quadFn E (z - x)) y = ((⟪z, y⟫ : ℝ) : EReal) + quadFn E y := by
  have hsub : ∀ u : E, ⟪u - z, y⟫ = ⟪u, y⟫ - ⟪z, y⟫ := fun u => by
    rw [← innerₗ_apply_apply, ← innerₗ_apply_apply, ← innerₗ_apply_apply, map_sub,
      LinearMap.sub_apply]
  rw [conj_apply]
  refine le_antisymm (iSup_le fun x => ?_) ?_
  · rw [quadFn_apply, quadFn_apply, ← _root_.EReal.coe_sub, ← _root_.EReal.coe_add,
      _root_.EReal.coe_le_coe_iff, innerₗ_apply_apply]
    have hrev : ‖z - x‖ ^ 2 = ‖x - z‖ ^ 2 := by rw [norm_sub_rev z x]
    linarith [norm_sub_sq_real (x - z) y, sq_nonneg ‖x - z - y‖, hsub x, hrev]
  · refine le_trans (le_of_eq ?_)
      (le_iSup (fun x : E => ((innerₗ E x y : ℝ) : EReal) - quadFn E (z - x)) (z + y))
    have harg : z - (z + y) = -y := by abel
    have hadd : ⟪z + y, y⟫ = ⟪z, y⟫ + ⟪y, y⟫ := by
      rw [← innerₗ_apply_apply, ← innerₗ_apply_apply, ← innerₗ_apply_apply, map_add,
        LinearMap.add_apply]
    rw [harg, quadFn_apply, quadFn_apply, ← _root_.EReal.coe_add, ← _root_.EReal.coe_sub,
      _root_.EReal.coe_eq_coe_iff, innerₗ_apply_apply, hadd, real_inner_self_eq_norm_sq, norm_neg]
    ring
...
theorem infConv_quadFn_apply (hb : ∀ x, f x ≠ ⊥) (z : E) :
    infConv f (quadFn E) z = ⨅ x, (f x + quadFn E (z - x)) := by
  rw [infConv_apply hb (fun x => quadFn_ne_bot x) z,
    ← iInf_comp_sub (fun y => f (z - y) + quadFn E y) z]
...
theorem moreau_add (hf : ClosedProperConvexFn f) (z : E) :
    infConv f (quadFn E) z + infConv (conj (innerₗ E) f) (quadFn E) z = quadFn E z := by
  obtain ⟨x₀, hx₀⟩ := hf.proper.dom_nonempty
  obtain ⟨y₀, hy₀⟩ := (proper_conj hf (B := innerₗ E)).dom_nonempty
  have hex : IsExactSum (innerₗ E) f (fun x => quadFn E (z - x)) :=
  ...
  have hD : infConv (conj (innerₗ E) f) (quadFn E) z
      = ⨅ y, (conj (innerₗ E) f y + quadFn E (z - y)) :=
    infConv_quadFn_apply (conj_ne_bot hf.proper.dom_nonempty) z
  have hP : infConv f (quadFn E) z
      = -(conj (innerₗ E) (f + fun x => quadFn E (z - x)) 0) := by
    rw [infConv_quadFn_apply hf.proper.ne_bot z, ← iInf_eq_neg_conj_zero (innerₗ E)]
    exact iInf_congr fun x => rfl
  have hstep : ∀ y : E,
      conj (innerₗ E) f (0 - -y) + conj (innerₗ E) (fun x => quadFn E (z - x)) (-y)
        = (conj (innerₗ E) f y + quadFn E (z - y)) + ((-(‖z‖ ^ 2 / 2) : ℝ) : EReal) := by
    intro y
    have hinner : ⟪z, -y⟫ = -⟪z, y⟫ := by
      rw [← innerₗ_apply_apply, ← innerₗ_apply_apply, map_neg]
    have hreal : ⟪z, -y⟫ + ‖y‖ ^ 2 / 2 = ‖z - y‖ ^ 2 / 2 + -(‖z‖ ^ 2 / 2) := by
      rw [hinner]
      have h := norm_sub_sq_real z y
      linarith
  ...
  have hQD : conj (innerₗ E) (f + fun x => quadFn E (z - x)) 0
      = infConv (conj (innerₗ E) f) (quadFn E) z + ((-(‖z‖ ^ 2 / 2) : ℝ) : EReal) := by
    rw [hex.conj_add_apply 0,
      iInf_comp_neg fun y => conj (innerₗ E) f (0 - y)
        + conj (innerₗ E) (fun x => quadFn E (z - x)) y,
      hD, Tdaf.EReal.iInf_add_coe]
    exact iInf_congr hstep
  have hDt : infConv (conj (innerₗ E) f) (quadFn E) z ≠ ⊤ := by
    rw [hD]
    obtain ⟨r, hr⟩ :=
      EReal.exists_coe_of_ne_bot_of_lt_top (conj_ne_bot hf.proper.dom_nonempty y₀) hy₀
    have hle : (⨅ y, (conj (innerₗ E) f y + quadFn E (z - y)))
        ≤ conj (innerₗ E) f y₀ + quadFn E (z - y₀) := iInf_le _ y₀
    rw [hr, quadFn_apply, ← _root_.EReal.coe_add] at hle
    exact ne_top_of_le_ne_top (_root_.EReal.coe_ne_top _) hle
  have hQb : conj (innerₗ E) (f + fun x => quadFn E (z - x)) 0 ≠ ⊥ := by
    intro hc
    have hle := sub_le_conj (innerₗ E) (f + fun x => quadFn E (z - x)) x₀ 0
  ...
  have hDb : infConv (conj (innerₗ E) f) (quadFn E) z ≠ ⊥ := by
    intro hc
    rw [hc] at hQD
...
theorem infConv_quadFn_ne_top (hf : ClosedProperConvexFn f) (z : E) :
    infConv f (quadFn E) z ≠ ⊤ := by
  intro hc
  have h := moreau_add hf z
...
theorem infConv_conj_quadFn_ne_top (hf : ClosedProperConvexFn f) (z : E) :
    infConv (conj (innerₗ E) f) (quadFn E) z ≠ ⊤ := by
  intro hc
  have h := moreau_add hf z
  rw [hc, _root_.EReal.add_top_of_ne_bot (infConv_quadFn_ne_bot hf z), quadFn_apply] at h
...
theorem mem_subgradient_iff_infConv_eq (hf : ClosedProperConvexFn f) {x y z : E}
    (hz : x + y = z) :
    y ∈ subgradient (innerₗ E) f x ↔
      f x + quadFn E y = infConv f (quadFn E) z ∧
        conj (innerₗ E) f y + quadFn E x = infConv (conj (innerₗ E) f) (quadFn E) z := by
  have hzx : z - x = y := by rw [← hz]; abel
  have hzy : z - y = x := by rw [← hz]; abel
  have hPle : infConv f (quadFn E) z ≤ f x + quadFn E y := by
    rw [infConv_quadFn_apply hf.proper.ne_bot z, ← hzx]
    exact iInf_le _ x
  have hDle : infConv (conj (innerₗ E) f) (quadFn E) z ≤ conj (innerₗ E) f y + quadFn E x := by
    rw [infConv_quadFn_apply (conj_ne_bot hf.proper.dom_nonempty) z, ← hzy]
    exact iInf_le _ y
  have hreal : ‖y‖ ^ 2 / 2 + ‖x‖ ^ 2 / 2 = ‖x‖ ^ 2 / 2 + ‖y‖ ^ 2 / 2 := by ring
  have hkey : (f x + quadFn E y) + (conj (innerₗ E) f y + quadFn E x)
      = (f x + conj (innerₗ E) f y) + ((‖x‖ ^ 2 / 2 + ‖y‖ ^ 2 / 2 : ℝ) : EReal) := by
    rw [add_add_add_comm, quadFn_apply, quadFn_apply, ← _root_.EReal.coe_add, hreal]
  have hwz : ((⟪x, y⟫ : ℝ) : EReal) + ((‖x‖ ^ 2 / 2 + ‖y‖ ^ 2 / 2 : ℝ) : EReal) = quadFn E z := by
    rw [← _root_.EReal.coe_add, quadFn_apply, ← hz, _root_.EReal.coe_eq_coe_iff]
    have h := norm_add_sq_real x y
    linarith
  constructor
  · intro hmem
    have hfen : f x + conj (innerₗ E) f y = ((⟪x, y⟫ : ℝ) : EReal) := by
      have h := hf.proper.mem_subgradient_iff_add_conj_eq.1 hmem
      rwa [innerₗ_apply_apply] at h
    have hsum : (f x + quadFn E y) + (conj (innerₗ E) f y + quadFn E x)
        = infConv f (quadFn E) z + infConv (conj (innerₗ E) f) (quadFn E) z := by
      rw [hkey, hfen, hwz, moreau_add hf z]
    obtain ⟨p, hp⟩ := EReal.exists_coe_of_ne_bot_of_lt_top (infConv_quadFn_ne_bot hf z)
      (lt_top_iff_ne_top.2 (infConv_quadFn_ne_top hf z))
    obtain ⟨q, hq⟩ := EReal.exists_coe_of_ne_bot_of_lt_top (infConv_conj_quadFn_ne_bot hf z)
      (lt_top_iff_ne_top.2 (infConv_conj_quadFn_ne_top hf z))
    have hAb : f x + quadFn E y ≠ ⊥ := by
      intro hcon
      rw [hcon, le_bot_iff, hp] at hPle
      exact absurd hPle (_root_.EReal.coe_ne_bot p)
    have hCb : conj (innerₗ E) f y + quadFn E x ≠ ⊥ := by
      intro hcon
    ...
    · rw [hcc, hq, _root_.EReal.coe_eq_coe_iff]
      linarith
  · rintro ⟨h1, h2⟩
    have hsum : (f x + conj (innerₗ E) f y) + ((‖x‖ ^ 2 / 2 + ‖y‖ ^ 2 / 2 : ℝ) : EReal)
        = ((‖z‖ ^ 2 / 2 : ℝ) : EReal) := by
      rw [← hkey, h1, h2, moreau_add hf z, quadFn_apply]
    rw [hf.proper.mem_subgradient_iff_add_conj_eq, innerₗ_apply_apply,
      eq_coe_of_add_coe_eq_coe hsum, _root_.EReal.coe_eq_coe_iff, ← hz]
    have h := norm_add_sq_real x y
    linarith
