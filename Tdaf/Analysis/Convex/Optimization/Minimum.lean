...
* `argmin_eq_singleton_iff_hasGradientAt_conj_zero`,
  `exists_argmin_eq_singleton_iff_differentiableAtFn_conj_zero` — **Theorem 27.1(e)**: the minimum
  set is the single vector `x` exactly when `f*` is differentiable at the origin with
  `∇f*(0) = ⟨·, x⟩`.
...
* `argmin_nonempty_and_isBounded_iff_zero_mem_interior_dom_conj` — **Theorem 27.1(d)**, with
  `zero_mem_interior_dom_conj_iff_recessionConeFn_eq_zero` for its second sentence, and
  `zero_mem_relint_dom_conj_iff_recessionConeFn_subset_constancySpace` — the last sentence of
  **Theorem 27.1(b)** — as its `ri` twin.
...
**Theorem 27.1(e) needs no reflexivity, and the note that said it did was wrong twice.** That note
read: *"It says that `∂f*(0)` is a singleton, which lives in `E**` and so needs a reflexive pairing
to be stated at all."* Neither half survives contact with the statement. The subdifferential in
question is `subgradient B.flip (conj B f) 0`, a subset of **`E`** — `conj B f : F → EReal` and
`B.flip : F →ₗ[ℝ] E →ₗ[ℝ] ℝ` is the pairing it is differentiated against, so a dual variable of
...
Both sentences of Theorem 27.1(i) are in `Duality/Level.lean`, as `zero_mem_closure_dom_conj_iff`
and `zero_notMem_closure_dom_conj_iff`, next to the Corollary 13.3.4 they specialise.
...
* R. T. Rockafellar, *Convex Analysis*, Princeton University Press, 1970, §27 (Theorem 27.1(a),
  (b), (c), (d), (e), (f), (g), (h), Theorem 27.2 with Corollaries 27.2.1–27.2.2, Theorem 27.3 with
...
omit [FiniteDimensional ℝ E] in
/-- **Rockafellar, Theorem 27.1(b)**, last sentence: the origin is in the *relative* interior of
`dom f*` exactly when every direction of recession of `f` is a direction in which `f` is constant.

The `ri` twin of `zero_mem_interior_dom_conj_iff_recessionConeFn_eq_zero`, and Corollary 13.3.4(b)
at the origin: `mem_relint_dom_conj_iff` says `0 ∈ ri (dom f*)` is `f 0⁺ ≥ 0` everywhere together
with strictness in every direction where `f 0⁺` is not additively reversible, and both conditions
collapse to `0⁺f ⊆ constancy space` through `le_recessionFn_of_neg_le` (Theorem 8.8), which is what
turns a bound on `(f0⁺)(-y)` into a lower bound on `(f0⁺)(y)`. -/
...
singleton exactly at a point of differentiability. Composing the two is the whole of clause (e).
The subdifferential in question is `subgradient B.flip (conj B f) 0`, a subset of `E` — the dual
variable of `f* : F → EReal` is an element of `E`, because that is what the pairing `B` says a
dual variable is. -/
...
Theorem 27.1(b) turns the hypothesis into `∂f*(0) = {x}`, and Theorem 25.1's converse half
(`hasGradientAt_evalCLM_of_subgradient_eq_singleton`, `Subgradient/Uniqueness.lean`) turns a lone
subgradient into a gradient. The finite-dimensionality is `F`'s, not `E`'s: it is the space `f*`
lives on, and it is where the normal-cone step of that converse runs. -/
...
/-- **Rockafellar, Theorem 27.1(e)**, sufficiency: if `f*` is differentiable at the origin with
`∇f*(0) = ⟨·, x⟩`, then `x` is the unique minimiser of `f`.

Theorem 25.2's sufficiency half (`subgradient_eq_singleton_of_dirDeriv_eq`) does the work, against
the directional derivative that `HasGradientAt.dirDeriv_eq` supplies. Properness of `f` is not
...
Finite-dimensionality of `F` is not used either: this half runs on Theorem 25.2's algebraic
sufficiency, and only the *necessity* half needs the normal-cone argument of
`Subgradient/Uniqueness.lean`. -/
...
/-- **Rockafellar, Theorem 27.1(e)**, in the book's own shape: the infimum of `f` is attained at a
*unique* point exactly when `f*` is differentiable at the origin.

The passage from the functional `∇f*(0) : F →L[ℝ] ℝ` to the vector `x : E` representing it is the
surjectivity half of `IsCompatiblePairing B.flip`, which is exactly the reflexivity Rockafellar
uses silently when he writes `x = ∇f*(0)` for a point of `ℝⁿ`. -/
