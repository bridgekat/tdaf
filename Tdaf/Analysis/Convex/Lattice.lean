...
`Tdaf.ConvexFns E = {f : E → EReal // ConvexFn f}` and proves that the instance computes what
Rockafellar says it computes. Almost nothing is proved here: the mathematics is already in
`Tdaf/Analysis/Convex/Operations/Hull.lean` (`Tdaf.gci_val_convHullFn`, the coreflection of all
functions onto the convex ones) and in `Tdaf/Analysis/Convex/Operations/Basic.lean`
(`Tdaf.convexFn_iSup`, Theorem 5.5). This file is the assembly.

## Main definitions

* `Tdaf.ConvexFns E` — the convex functions on `E`, bundled as a type.
* `Tdaf.ConvexFns.instCompleteLattice` — the complete lattice structure, obtained from
  `Tdaf.gci_val_convHullFn` by `GaloisCoinsertion.liftCompleteLattice`.
* `Tdaf.ConvexFns.coeOrderEmbedding` — the coercion `ConvexFns E → (E → EReal)` as an
  `OrderEmbedding`: the order is the pointwise order and nothing else.
* `Tdaf.ConvexFns.coeSSupHom` — the same coercion as an `sSupHom`. There is deliberately no
  `sInfHom`, and `Tdaf.ConvexFns.not_coe_inf_eq_inf` says there cannot be one.

## Main results

* `Tdaf.ConvexFns.coe_sSup`, `Tdaf.ConvexFns.coe_iSup`, `Tdaf.ConvexFns.coe_sSup_apply`,
  `Tdaf.ConvexFns.coe_sup` — **the join is the pointwise supremum**, with no hull taken. This is
  Rockafellar's "`sup {f i}`", and it is exactly Theorem 5.5 (`Tdaf.convexFn_iSup`) that makes it
  work.
* `Tdaf.ConvexFns.coe_sInf`, `Tdaf.ConvexFns.coe_iInf`, `Tdaf.ConvexFns.coe_inf` — **the meet is
  the convex hull**: `⨅ i, f i` is `Tdaf.convFn` and `f ⊓ g` is `Tdaf.convFn₂`, not the pointwise
  infimum.
* `Tdaf.ConvexFns.coe_top`, `Tdaf.ConvexFns.coe_bot` — the extreme elements are the constants `⊤`
  and `⊥`, both of which are convex, so again no hull is taken.
* `Tdaf.ConvexFns.exists_coe_inf_lt_inf` and `Tdaf.ConvexFns.not_coe_inf_eq_inf` — **the meet
  really is not the pointwise infimum**, on the witness of `Tdaf.convFn₂_indicatorFn_lt_inf`.
...
functions*: `Tdaf.convFn₂_indicatorFn_lt_inf` exhibits `δ(·|{0})` and `δ(·|{1})` on `ℝ`, whose
...
which is `conv {f i}` by its universal property `Tdaf.isGreatest_convFn`. Rockafellar's parenthesis
...
(`NOTES.md` gotcha 27, and the design note in `Operations/Hull.lean`). The honestly monotone
coreflection `Tdaf.gci_val_convHullFn` supplies `CompleteLattice` directly.

**Why an `abbrev`.** `Tdaf.ConvexFns E` is a reducible abbreviation for the very subtype that
`Tdaf.gci_val_convHullFn` is stated about, so the coinsertion applies to it on the nose, the
...
(`Tdaf.ConvexFns.coeOrderEmbedding` is `OrderEmbedding.subtype`, with `Iff.rfl` for its map
...
/-! ### The pointwise infimum and the binary convex hull

The one lemma this file has to prove for itself: the coreflector `Tdaf.convHullFn` applied to a
pointwise minimum is the binary convex hull. Both sides are the greatest convex function below `f`
and below `g`, so this is two applications of the universal properties. -/
...
This is a reducible abbreviation for the subtype `Tdaf.gci_val_convHullFn` is stated about, so the
...
The instance is not built by hand: it is the coreflection `Tdaf.gci_val_convHullFn` of all
...
operation on `E → EReal` followed by `Tdaf.convHullFn`, and the lemmas below simply evaluate that
...
/-- `Tdaf.ConvexFns.coeOrderEmbedding` is the coercion. -/
@[simp]
...
Nothing is lost on the way up. `Tdaf.convexFn_iSup` (Theorem 5.5) says the pointwise supremum of
...
/-- The indexed form of `Tdaf.ConvexFns.coe_sSup`: an `⨆` of convex functions is computed
pointwise. -/
...
/-- `Tdaf.ConvexFns.coe_sSup` evaluated at a point. -/
...
companion `sInfHom` — see `Tdaf.ConvexFns.not_coe_inf_eq_inf`. -/
noncomputable def coeSSupHom : sSupHom (ConvexFns E) (E → EReal) where
  toFun := Subtype.val
  map_sSup' := coe_sSup

/-- `Tdaf.ConvexFns.coeSSupHom` is the coercion. -/
@[simp]
...
functions is their convex hull `Tdaf.convFn` — *not* their pointwise infimum, which is generally
not convex. -/
@[simp]
...
/-- The binary case: the meet of two convex functions is `Tdaf.convFn₂`, their binary convex
hull. -/
@[simp]
...
`Tdaf.ConvexFns.exists_coe_inf_lt_inf`. -/
...
`Tdaf.convFn₂_indicatorFn_lt_inf` is the machine-checked witness: on `ℝ`, the indicator functions
...
/-- **The meet of `Tdaf.ConvexFns` is strictly below the pointwise infimum, in general.** -/
...
homomorphism: it does not commute with binary meets. Contrast `Tdaf.ConvexFns.coeSSupHom`. -/
