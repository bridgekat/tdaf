...
* `span_eq_sub_of_isCone` — the subspace generated by a convex cone containing the origin is
  `s - s` (**Theorem 2.7**).
* `smul_closure_eq_of_isCone` — the closure of a cone is a cone (**§6**, book line 2103). This is
  the one topological statement in the file, and it needs only that each individual scaling is
  continuous, not a topological vector space.
...
differences.** Every element of the span is a finite linear combination, and closure under addition
and positive scaling collects the positive terms into one element of `s` and the negative terms into
another.

Together with `vectorSpan_eq_span_of_zero_mem` (`Tdaf/LinearAlgebra/Subspace.lean`) this is
**Rockafellar, Theorem 2.7**: for a convex cone containing the origin, the span, the affine hull and
`s - s` all coincide. -/
...
/-- **Rockafellar, §6 (book line 2103): the closure of a cone is a cone.** Multiplication by a
non-zero scalar is a homeomorphism, so it commutes with `closure`, and the cone condition then
transports verbatim.

Convexity plays no part, and neither does the closure being taken in a vector space: only the
continuity of the individual scalings is used. -/
