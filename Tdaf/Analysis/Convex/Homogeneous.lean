...
*subadditivity* (Rockafellar's Theorem 4.7), and the theory of support functions (§13) and gauges
(§15) rests on that equivalence.
...
proper. This is why Corollary 4.7.1 carries a nonemptiness hypothesis on the index set — the empty
sum would assert `f 0 ≤ 0` — and why the spanning-set form of Theorem 4.8 carries one too.
...
* `convex_iff_add_mem_of_isCone` — **Theorem 2.6**: a cone is convex iff it is closed under
  addition. This is Mathlib's `ConvexCone` content, repackaged as a statement about bare sets.
* `span_eq_sub_of_isCone` — the subspace generated by a convex cone containing the origin is
  `s - s` (**Theorem 2.7**).
* `smul_closure_eq_of_isCone` — the closure of a cone is a cone (§6). The one topological
...
* `PosHomogeneous.sum_le`, `PosHomogeneous.neg_le` — **Corollaries 4.7.1 and 4.7.2**.
* `PosHomogeneous.isLinearOn_iff`, `PosHomogeneous.exists_linearMap_iff` — **Theorem 4.8**:
...
Rockafellar states Theorem 4.7 for `f` with values in `(-∞, +∞]`; that is kept inline as
`∀ x, f x ≠ ⊥`. Positive homogeneity is not stated with a scalar action on `EReal` — there is no
`SMul ℝ EReal` instance — so `(a : EReal) * z` is used throughout.
...
/-- **Rockafellar, Theorem 2.6.** A cone is convex if and only if it is closed under addition. The
"only if" direction is the observation that `x + y = 2 * ((x + y) / 2)`. -/
...
`vectorSpan_eq_span_of_zero_mem` this is **Theorem 2.7**: span, affine hull and `s - s` coincide. -/
...
/-- **The closure of a cone is a cone** (Rockafellar §6). Multiplication by a non-zero scalar is a
...
`f (a • x) = a * f x` for every `a > 0` (Rockafellar §4). Only *positive* scalars are constrained;
...
is convex if and only if it is subadditive: subadditivity of `f` is precisely closure of the cone
`epi f` under addition, so this is Theorem 2.6. -/
...
extends to positive linear combinations. The index set must be nonempty: the empty sum would
...
on a nonempty set `s`, it is odd on the whole subspace spanned by `s`. The nonemptiness hypothesis
is not decoration: it is what supplies `f 0 = 0`, which the book's proof uses without comment when
a coefficient `λᵢ` vanishes. -/
...
which makes Mathlib's `ConvexCone` API available to §13 (support functions) and §14 (polarity). No
hypothesis on the values of `f` is needed: closure under addition comes from Theorem 2.6, not from
subadditivity. -/
